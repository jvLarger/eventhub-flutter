...
class EventoCadastroGaleria extends StatefulWidget {
  final Function(Arquivo) onUpload;
  final Function(int) onRemove;
  final List<EventoArquivo> listaEventoArquivo;

  const EventoCadastroGaleria({
    super.key,
    required this.listaEventoArquivo,
    required this.onUpload,
    required this.onRemove,
  });

  @override
  State<EventoCadastroGaleria> createState() => _EventoCadastroGaleriaState();
}

class _EventoCadastroGaleriaState extends State<EventoCadastroGaleria> {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: widget.listaEventoArquivo.length,
            ...
            return index % 3 == 0
                ? Row(
                    children: [
                      ...
                      (index + 1 <= widget.listaEventoArquivo.length - 1 ? montarCardImagem(index + 1) : const SizedBox()),
                      const SizedBox(
                        width: 15,
                      ),
                      (index + 2 <= widget.listaEventoArquivo.length - 1 ? montarCardImagem(index + 2) : const SizedBox()),
                    ],
                  )
                : const SizedBox();
          },
      ...
      child: Stack(
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(15),
            child: Image.network(
              Util.montarURlFotoByArquivo(widget.listaEventoArquivo[index].arquivo!),
              fit: BoxFit.cover,
              width: widthCard,
              height: widthCard,
            ),
